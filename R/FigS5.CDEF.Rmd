...
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_file = file.path("..", "results", "FigS5.CDEF.html"))
  })
---
...
*The purpose of this file is to reproduce results from the associated paper, specifically sections C, D, E, and F of Supplementary Figure 5.*
...
# ------------------------------------------------------------------------------------------------------------
file_list <- list.files(file.path(data_dir, "zetaview"))
file_list <- file_list[-grep("10K", x = file_list)] # Remove 10K pellets.

data <- data.frame(matrix(nrow = 1200, ncol = length(file_list)) + 1)
for (i in seq_along(file_list)) {

  file <- read.delim(file.path(data_dir, "zetaview", file_list[i]), skip = 70) # Skip metadata lines.
  if (i == 1) {
  ...
  # --------------------------------------------------------------------------------------------------------------
  dilution <- read.delim(file.path(data_dir, "zetaview", file_list[i]), FALSE, nrows = 1, skip = 24)
...
```{r, results = "hide", fig.show = "hide", message = FALSE, warning = FALSE}
# Create a dataset where replicates of each fraction are aggregated together by their mean.
# -----------------------------------------------------------------------------------------
...
# Convert concentration values into proportion values.
# ----------------------------------------------------
fraction_data_proportion <- fraction_data
for (i in seq_along(colnames(fraction_data_proportion))) {
  if (i != 1) { # The first column contains sizes not concentrations.
...
# ---------------------------
data_sub <- fraction_data_proportion[1:which(fraction_data_proportion$size == 297.5), ] # Values level off past ~300 nm.
...
```{r, results = "hide", fig.show = "hide", message = FALSE, warning = FALSE}
# Convert concentration values into percentage values.
# ----------------------------------------------------
...
ggplot(molten_data, aes(Fraction, Diameter)) +
  ...
  stat_summary(fun = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..), width = 0.75) +
  geom_beeswarm(aes(fill = Donor), size = 2, pch = 21, cex = 2) +
